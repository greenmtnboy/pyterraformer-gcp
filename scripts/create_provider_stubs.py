...
def _create_stubs(provider: str, version: str, output_path: str, terraform_path: str):
    ...
    workspace.save_all()
    tf.run(['init', '-upgrade'], path=os.getcwd())
...
@click.command()
@click.option('--package', help = 'The package name, eg. hashicorp/aws to create stubs for')
...
def create_stubs(provider:str, version:str, output_path:str, terraform_path:str):
    _create_stubs(provider=provider, version=version, output_path=output_path, terraform_path=terraform_path)
