...
# this block can contain multiple items, items in a list are required
@dataclass 
class ExplicitBucketsItem():
        bounds:List[float]
        # non-optional-blocks
        
# wrapper list class
class ExplicitBuckets(BlockList):
        items: List[ExplicitBucketsItem]
...
class BucketOptionsItem():
        # non-optional-blocks
        explicit_buckets: Optional[ExplicitBuckets]=None,
...
class GoogleLoggingMetric(ResourceObject):
    """    
    Args:
        filter (str): An advanced logs filter (https://cloud.google.com/logging/docs/view/advanced-filters) which
                    is used to match log entries.
        name (str): The client-assigned metric identifier. Examples - "error_count", "nginx/requests".
    ...
    def __init__(self,
        ...
        timeouts: Optional[Timeouts]=None,
            ...
            if explicit_buckets is not None:
                kwargs['explicit_buckets'] = explicit_buckets
