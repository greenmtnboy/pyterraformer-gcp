...
class GoogleContainerAwsNodePool(ResourceObject):
    ...
    Args:
        ...
        subnet_id (str): Required. The subnet where the node pool node run.
        version (str): Required. The Kubernetes version to run on this node pool (e.g. `1.19.10-gke.1000`). You can list all supported versions on a given Google Cloud region by calling GetAwsServerConfig.
    """
